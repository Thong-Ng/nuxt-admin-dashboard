...
<template>
  <SidebarMenu v-if="sidebar.state.value === 'expanded'">
    <Collapsible
      :key="item.title"
      ...
      <SidebarMenuItem>
        <CollapsibleTrigger as-child>
          <SidebarMenuButton :tooltip="item.title" :size="size">
            ...
            <Icon v-if="sidebar.state.value === 'expanded'" name="i-lucide-chevron-right" class="ml-auto transition-transform duration-200 group-data-[state=open]/collapsible:rotate-90" />
          </SidebarMenuButton>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <SidebarMenuSub>
            ...
            >
              <SidebarMenuSubButton as-child>
                <NuxtLink :to="subItem.link" @click="setOpenMobile(false)">
                   <span v-if="sidebar.state.value === 'expanded'">{{ subItem.title }}</span> 
  ...
  <SidebarMenu v-if="sidebar.state.value === 'collapsed'">
    <Popover
      :key="item.title"
      v-model:open="openCollapsible"
      as-child
    ...
    >
      <SidebarMenuItem>
        ...
        <PopoverContent class="m-4 mt-0 bg-[#27548A]">
          <SidebarMenuSub>
            <SidebarMenuSubItem
              v-for="subItem in item.children"
              :key="subItem.title"
